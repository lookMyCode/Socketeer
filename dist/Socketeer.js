        ...
        this.webSocketServer.on('connection', async (ws, request) => {
            ...
            if (this.rateLimitConfig?.maxConnections) {
                if (this.webSocketServer.clients.size > this.rateLimitConfig.maxConnections) {
                    ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.SERVICE_UNAVAILABLE.code, SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.SERVICE_UNAVAILABLE.status);
                    return;
                }
            }
            // 1. Create Context
            ...
            try {
                try {
                    ...
                    if (accessDenied) {
                        ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.ACCESS_DENIED.code, SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.ACCESS_DENIED.status);
                        return;
                    }
                }
                catch (err) {
                    if (err instanceof Error) {
                        ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.ACCESS_DENIED.code, err.message);
                    }
                    else {
                        ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.ACCESS_DENIED.code, SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.ACCESS_DENIED.status);
                    }
                ...
                if (this.openedControllers[currentPath]) {
                    ...
                    if (!currentRoute) {
                        ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.NOT_FOUND.code, SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.NOT_FOUND.status);
                        return;
                    }
                ...
                if (onConnect) {
                    try {
                        ...
                        if (err instanceof Error) {
                            ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.INTERNAL_SERVER_ERROR.code, SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.INTERNAL_SERVER_ERROR.status);
                        }
                        else {
                            ws.close(SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.INTERNAL_SERVER_ERROR.code, SOCKETEER_STATUSES_1.SOCKETEER_STATUSES.INTERNAL_SERVER_ERROR.status);
                        }
